...
describe("Merge Sort", () => {
  it("if tempArr equals [[38],[27],[43],[10]]", () => {
    ...
    expect(tempArr).toEqual([[38], [27], [43], [10]]);
  });

  it("if result array equals [ 10, 27, 38, 43 ] after merge sort", () => {
    for (let i = 0; i < arr.length; i++) {
      let half = i >= arr.length / 2;
      if (half) tempArr[1].push(arr[i]);
      else tempArr[0].push(arr[i]);
    ...
    expect(result).toEqual([10, 27, 38, 43]);
  });
});
